...
import { fetchTicketsTicket, deleteTicketsTicket } from '../../reducers/TicketsTicket/Actions'
import { connect } from 'react-redux'
import TicketsTicketModel from 'models/TicketsTicketModel'
...
class TicketsTicketList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let TicketsTicket = {}
		if (AllFilters.TicketsTicket !== undefined){
			TicketsTicket = AllFilters.TicketsTicket
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (TicketsTicket[ColumnName] !== undefined){
				Param = TicketsTicket[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.TicketsTicket = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = TicketsTicketModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchTicketsTicket(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = TicketsTicketModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/TicketsTicket/NewItem/TicketsPromoter/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/TicketsTicket/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/TicketsTicket/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="TicketsTicket" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		Payload: state.TicketsTicketReducer.item,
		Payloads: state.TicketsTicketReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchTicketsTicket, deleteTicketsTicket})(TicketsTicketList);