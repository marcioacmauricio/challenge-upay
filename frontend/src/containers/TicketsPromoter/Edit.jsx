...
import { createTicketsPromoter, readTicketsPromoter, updateTicketsPromoter } from 'reducers/TicketsPromoter/Actions'
import TicketsPromoterModel from 'models/TicketsPromoterModel'
import Fields from 'fields/Fields'
import { MenuHeader, HeaderAdmin } from 'components/Headers'
class TicketsPromoterEdit extends React.Component {
	constructor() {
		super()
		this.ColumnsList = ['id', 'ordering', 'state', 'checked_out', 'checked_out_time', 'created_by', 'created_time', 'modified_by', 'modified_time', 'name']
		this.ColumnsFields = {}
		let ColumnData = {}
		let Item = {}
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateTicketsPromoter(Values);
		} else {
			this.props.createTicketsPromoter(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/TicketsPromoter/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
								    ...
								    <Link className="btn btn-outline-info" to="/Admin/TicketsPromoter/ListItems"><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readTicketsPromoter, createTicketsPromoter, updateTicketsPromoter})(TicketsPromoterEdit);