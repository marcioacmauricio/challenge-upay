...
import { fetchTicketsCartItem, deleteTicketsCartItem } from '../../reducers/TicketsCartItem/Actions'
...
class TicketsCartItemList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_cart', 'id_event']
		if(this.ColumnsList.indexOf('id_cart') === -1){
			this.ColumnsList.push('id_cart')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let TicketsCartItem = {}
		if (AllFilters.TicketsCartItem !== undefined){
			TicketsCartItem = AllFilters.TicketsCartItem
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (TicketsCartItem[ColumnName] !== undefined){
				Param = TicketsCartItem[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.TicketsCartItem = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = TicketsCartItemModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchTicketsCartItem(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = TicketsCartItemModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/TicketsCartItem/NewItem/TicketsCart/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/TicketsCartItem/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/TicketsCartItem/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="TicketsCartItem" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.TicketsCartItemReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchTicketsCartItem, deleteTicketsCartItem})(TicketsCartItemList);