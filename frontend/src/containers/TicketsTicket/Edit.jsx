...
import { createTicketsTicket, readTicketsTicket, updateTicketsTicket } from 'reducers/TicketsTicket/Actions'
import TicketsTicketModel from 'models/TicketsTicketModel'
import Fields from 'fields/Fields'
import { MenuHeader, HeaderAdmin } from 'components/Headers'
class TicketsTicketEdit extends React.Component {
	constructor() {
		super()
		this.ColumnsList = ['id', 'ordering', 'state', 'checked_out', 'checked_out_time', 'created_by', 'created_time', 'modified_by', 'modified_time', 'id_promoter', 'price']
		this.ColumnsFields = {}
		let ColumnData = {}
		let Item = {}
		...
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			ColumnData = TicketsTicketModel.columns[ColumnName]
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateTicketsTicket(Values);
		} else {
			this.props.createTicketsTicket(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
		if (this.props.match.params.id > 0){
			...
			this.props.readTicketsTicket( Posts );
		} else {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/TicketsTicket/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
	...
	renderField(ColumnName){
		let ColumnData = TicketsTicketModel.columns[ColumnName]
		return (
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/TicketsTicket/ListItems/TicketsPromoter/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readTicketsTicket, createTicketsTicket, updateTicketsTicket})(TicketsTicketEdit);