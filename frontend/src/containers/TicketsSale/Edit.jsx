...
import { createTicketsSale, readTicketsSale, updateTicketsSale } from 'reducers/TicketsSale/Actions'
import TicketsSaleModel from 'models/TicketsSaleModel'
import Fields from 'fields/Fields'
import { MenuHeader, HeaderAdmin } from 'components/Headers'
class TicketsSaleEdit extends React.Component {
	constructor() {
		super()
		this.ColumnsList = ['id', 'ordering', 'state', 'checked_out', 'checked_out_time', 'created_by', 'created_time', 'modified_by', 'modified_time', 'id_promoter', 'amount', 'total']
		this.ColumnsFields = {}
		let ColumnData = {}
		let Item = {}
		...
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			ColumnData = TicketsSaleModel.columns[ColumnName]
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateTicketsSale(Values);
		} else {
			this.props.createTicketsSale(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
		if (this.props.match.params.id > 0){
			...
			this.props.readTicketsSale( Posts );
		} else {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/TicketsSale/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
	...
	renderField(ColumnName){
		let ColumnData = TicketsSaleModel.columns[ColumnName]
		return (
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/TicketsSale/ListItems/TicketsUser/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readTicketsSale, createTicketsSale, updateTicketsSale})(TicketsSaleEdit);